            ...
            <div className="bg-linear-to-r from-orange-50 to-amber-50 p-6 flex flex-col md:flex-row gap-6 items-start">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {(item.groups?.length ? item.groups : item.group ? [item.group] : []).map((g: string) => (
                    <Badge key={g} variant="outline" className="bg-white/50">
                      {g}
                    </Badge>
                  ))}
                  {(item.forms?.length ? item.forms : item.form ? [item.form] : []).map((f: string) => (
                    <Badge key={f} variant="outline" className="bg-white/50">
                      {f}
                    </Badge>
                  ))}
                  {(item.toxicityLevels?.length ? item.toxicityLevels : item.toxicityLevel ? [item.toxicityLevel] : []).map((tox: string) => (
                    <span
                      key={tox}
                      className={`px-2 py-0.5 rounded text-xs font-semibold border ${getControlLevelColor(tox)}`}
                    >
                      {tox}
                    </span>
                  ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {(!item.forms?.length && !item.form) && <span className="text-sm text-slate-400">N/A</span>}
                  </div>
                </div>
                <div>
                  <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {(item.actionTypes?.length ? item.actionTypes : item.actionType ? [item.actionType] : []).map((a: string) => (
                      <Badge key={a} variant="outline" className="text-xs bg-slate-50">{a}</Badge>
                    ))}
                    {(!item.actionTypes?.length && !item.actionType) && <span className="text-sm text-slate-400">N/A</span>}
                  </div>
                </div>
                {((item.toxicityLevels?.length ?? 0) > 0 || item.toxicityLevel) && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(item.toxicityLevels?.length ? item.toxicityLevels : [item.toxicityLevel]).map((tox: string) => (
                        <span
                          key={tox}
                          className={`px-2 py-1 rounded text-xs font-semibold border ${getControlLevelColor(tox)}`}
                        >
                          {tox}
                        </span>
                      ))}
