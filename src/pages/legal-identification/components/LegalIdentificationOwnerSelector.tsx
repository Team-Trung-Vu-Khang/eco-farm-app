...
type LegalIdentificationOwnerSelectorProps = {
...
export function LegalIdentificationOwnerSelector({
  enterprises,
  value,
  onChange,
  disabled = false,
}: LegalIdentificationOwnerSelectorProps) {
  ...
  const selectedEnterprise = useMemo(() => {
    return (
      enterprises.find((enterprise) => enterprise.name === value) ||
      enterprises.find((enterprise) => enterprise.id.toString() === value)
    );
  }, [enterprises, value]);
  ...
  const filteredEnterprises = useMemo(() => {
    ...
    return enterprises.filter((enterprise) => {
      return (
        ...
        enterprise.brandName?.toLowerCase().includes(keyword) ||
        enterprise.taxCode.toLowerCase().includes(keyword)
      );
    });
  ...
  const handleConfirm = () => {
    const selected = enterprises.find(
      (enterprise) => enterprise.id.toString() === tempSelectedId,
    );
    if (selected) {
  ...
  const handleCancel = () => {
    setTempSelectedId(selectedEnterprise?.id.toString() || "");
    setSearchTerm("");
    setIsOpen(false);
  };
  ...
  return (
    <>
      <div
        className={[
          "group min-h-10 cursor-pointer rounded-lg border p-4 transition-all",
          selectedEnterprise
            ? "border-slate-200 bg-white"
            : "border-dashed border-slate-300 bg-slate-50",
          disabled ? "cursor-default opacity-80" : "hover:shadow-sm",
        ].join(" ")}
        onClick={() => {
          if (disabled) return;
          setTempSelectedId(selectedEnterprise?.id.toString() || "");
        ...
        {selectedEnterprise ? (
          ...
          <div className="flex w-full flex-col items-center justify-center gap-3 py-4 text-muted-foreground transition-all group-hover:text-primary">
            ...
            <div className="text-[11px] opacity-60">
              Thông tin đơn vị sẽ được dùng cho hồ sơ pháp lý
            </div>
          </div>
        )}
      </div>
