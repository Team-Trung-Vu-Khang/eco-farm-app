...
type Props = {
  ...
  updateAnimalLocation: (id: string, lat: number, lng: number) => void;
...
export const AnimalDistributionGpsStep = ({
  ...
  const activeAnimalLocation = useMemo(() => {
    return (
      animalLocations.find((location) => location.id === selectedAnimalId) ??
      animalLocations[0] ??
      null
    );
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-lg p-5 flex items-start gap-4">
        ...
        <div className="text-purple-900">
          ...
          <div className="text-sm opacity-90">
            Xác định và điều chỉnh tọa độ GPS cho từng vật nuôi. Bạn có thể kéo
      ...
      {animalLocations.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 border-2 border-dashed rounded-2xl bg-slate-50 text-slate-500">
          ...
          <p className="text-muted-foreground text-center max-w-md mb-6">
            ...
            <strong>{totalAnimals}</strong> vật nuôi dựa trên phương thức phân
            bổ đã chọn.
          </p>
          <Button
            onClick={generateAnimalLocations}
            size="lg"
            className="bg-purple-600 hover:bg-purple-700 shadow-lg shadow-purple-200"
          >
            <Navigation className="w-5 h-5 mr-2" />
            Khởi tạo {totalAnimals} điểm GPS
          </Button>
        </div>
      ) : (
            ...
            <Card className="flex-1 border-none shadow-lg overflow-hidden flex flex-col relative ring-1 ring-slate-200">
              ...
              <div className="absolute bottom-4 left-4 right-auto bg-white/90 backdrop-blur-md p-3 rounded-lg shadow-lg border border-slate-200 max-w-[200px] z-[500]">
                ...
                <div className="space-y-1.5 max-h-[100px] overflow-y-auto pr-1 custom-scrollbar">
                  {Array.from(
                    new Set(animalLocations.map((location) => location.seedId)),
                  ).map((seedId) => {
                    const seed = MOCK_SEEDS.find((item) => item.id === seedId);
                    const count = animalLocations.filter(
            ...
            <Card className="flex-none bg-indigo-900 text-white border-none shadow-md overflow-hidden relative">
              ...
              <CardContent className="p-5 relative z-10">
                ...
                <div className="flex items-end gap-2 mb-4">
                  <span className="text-3xl font-bold">{totalAnimals}</span>
                  ...
                  <ScrollArea className="h-full">
                    <div className="divide-y divide-slate-100">
                      {animalLocations.map((location) => {
                        ...
                        const isSelected = selectedAnimalId === location.id;

                        return (
                          <div
                            key={location.id}
                            onClick={() => setSelectedAnimalId(location.id)}
