...
import { type AnimalDistributionMethod, type AnimalDistributionScope, MOCK_SEEDS, type AnimalEntry, type AnimalLocation, type RowConfig } from "../constants";
...
export const AnimalDistributionConfirmationStep = ({
  scope,
  distributionMethod,
  selectedRegion,
  selectedAreas,
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 max-w-4xl mx-auto">
      <div className="bg-green-50 border border-green-100 rounded-2xl p-8 text-center relative overflow-hidden">
        ...
        <p className="text-green-700/80 mt-2 z-10 relative max-w-lg mx-auto">
          Kiểm tra kỹ thông tin trước khi lưu. Hệ thống sẽ tạo {totalAnimals} cây
          ...
          <div className="p-0">
            <table className="w-full text-sm">
              <tbody>
                ...
                <tr className="border-b">
                  <td className="py-3 px-4 text-muted-foreground">Giống vật nuôi</td>
                  <td className="py-3 px-4 font-medium">
                    {
                      Array.from(
                        new Set(
                          (distributionMethod === "zone" ? animalEntries : rowConfigs)
                ...
                <tr className="border-b">
                  <td className="py-3 px-4 text-muted-foreground">Tổng số con</td>
      ...
      {distributionMethod === "zone" && animalEntries.length > 0 && (
        <Card className="border-slate-200 shadow-sm">
          <div className="bg-slate-50 border-b p-4">
            <h4 className="font-semibold text-slate-800">Chi tiết vật nuôi</h4>
          </div>
              ...
              {animalEntries.map((entry, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Badge variant="outline">{entry.quantity} con</Badge>
                  </div>
                );
              })}
            </div>
              ...
              {rowConfigs.map((row) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Badge variant="outline">{row.quantity} con</Badge>
                  </div>
                );
              })}
            </div>
