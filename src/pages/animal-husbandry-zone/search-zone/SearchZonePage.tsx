...
import { CultivationRegionDetailView } from "../animal-husbandry-region/AnimalHusbandryRegionDetailPage";
import { mapSeedToBreed } from "../animal-husbandry-region/constants";
...
const SearchZonePage = () => {
  ...
  // Map region crops to breeds for animal husbandry context
  const mappedRegions = useMemo(() => {
    return regions.map((region) => ({
      ...region,
      cropVarieties: (region.cropVarieties || []).map((cv) => {
        const numericId = parseInt(String(cv.id).replace(/\D/g, "")) || 0;
        const mapped = mapSeedToBreed({ id: numericId, origin: cv.seedType });
        return {
          ...cv,
          name: mapped.crop.name, // e.g. "Heo", "Bò"
          variety: mapped.cropVariety.varietyName, // e.g. "Heo Yorkshire"
          seedType: mapped.origin, // e.g. "Đan Mạch"
        };
      }),
    }));
  }, [regions]);
  ...
  // Get main animal: prefer configs, then fallback to first cropVariety on the unit
  ...
  // Get unique values for filters from real mapped regions
  ...
  const uniqueWards = Array.from(
    new Set(mappedRegions.map((r) => r.ward).filter((w): w is string => !!w)),
  );
  const allCropVarieties = mappedRegions.flatMap(
  ...
  const handleSearch = () => {
    toast({
      ...
      description: `Đã tìm thấy ${totalCultivationRegions} vùng chăn nuôi phù hợp.`,
    });
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
              {enterpriseRegions.length} vùng chăn nuôi
            </p>
          </div>
          <ChevronRight
            size={14}
        ...
        <div className="bg-white border-b p-4 z-40 shadow-sm rounded-md">
          <div className="w-full flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="relative flex-1 w-full">
              ...
              <Input
                placeholder="Tìm kiếm vùng chăn nuôi theo tên, đối tượng chăn nuôi..."
            ...
            <div className="relative z-10 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-green-700/80 font-medium">
                  ...
                  vùng chăn nuôi phù hợp với tiêu chí của bạn.
                </p>
              </div>
            </div>
            ...
            {!selectedEnterpriseId ? (
              <div className="flex-1 flex flex-col items-center justify-center opacity-40">
                ...
                <h3 className="text-xl font-bold text-slate-400 uppercase tracking-widest text-center px-10">
                  Chọn đơn vị sở hữu để xem vùng chăn nuôi
                </h3>
              </div>
            ) : (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                      Chi tiết bản đồ & Danh sách vùng chăn nuôi
                    </p>
                  </div>
                </div>
                          ...
                          <div className="flex-1 overflow-y-auto split-scrollbar p-5">
                            {selectedUnit ? (
                              ...
                              <div className="h-full flex flex-col items-center justify-center text-slate-300 italic text-center text-sm px-4">
                                ...
                                />
                                Chọn vùng chăn nuôi (bảng danh sách) hoặc chọn
                ...
                <div className="flex-1 bg-white rounded-xl overflow-hidden flex flex-col">
                  <div className="flex items-center justify-between px-2  bg-slate-50/50 border-b ">
                    <div className="p-4 font-black text-xs uppercase tracking-widest text-slate-500">
                      Danh sách vùng chăn nuôi
                    </div>
                    <Badge variant="outline" className="font-bold">
                      {filteredCultivationRegions.length} vùng chăn nuôi
                    </Badge>
                  </div>
