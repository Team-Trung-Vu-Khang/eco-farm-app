...
import {
  type PlantEntry,
  makeEmptyPlant,
} from "../../animal-husbandry-region/components/types";
...
export const plantEntrySchema = z.object({
  ...
  plotId: z.string().min(1, "Vui lòng chọn vị trí chăn nuôi cho vật nuôi"),
  coordinate: z.object({
    lat: z.number(),
    lng: z.number(),
  }),
...
export const plantFormSchema = z.object({
  cultivationRegionId: z.string().min(1, "Vui lòng chọn vùng chăn nuôi"),
  selectedScopeIds: z
    .array(z.string())
    .min(1, "Vui lòng chọn ít nhất một phạm vi"),
  plants: z
    .array(plantEntrySchema)
    .min(1, "Vui lòng thêm ít nhất một vật nuôi"),
});
...
export const usePlantIdentificationForm = ({
  ...
  const validateAndSnapToUnit = (
    _plantEntryId: string,
    _lat: number,
    _lng: number,
  ) => {
    // No-op for mapless chăn nuôi
  };
  ...
  const handleComplete = () => {
    ...
    const newPlantArr = plants.map((p, index) => {
      ...
      return {
        ...initialData,
        code: generatedCode,
        name: initialData?.name || plotName || `Vật nuôi ${generatedCode}`,
        type: initialData?.type || "Vật nuôi",
