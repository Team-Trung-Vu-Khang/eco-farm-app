      ...
      <DialogContent className="max-w-xl p-0 overflow-hidden rounded-2xl border-none shadow-2xl flex flex-col max-h-[90vh]">
        <DialogHeader className="p-6 bg-slate-50 border-b shrink-0">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Chọn các con giống cụ thể thuộc giống vật nuôi này.
          </p>
        </DialogHeader>
      ...
      <div className="flex items-center justify-between">
        ...
        {isChecked && showSeedSelection && useSpecificSeeds && (
          <Button
            ...
            className="text-[11px] font-bold text-amber-700 border-amber-200 hover:bg-amber-50 rounded-md py-0.5 px-2 h-7"
          >
            Chọn con giống
          </Button>
        )}
          ...
          {selectedSeedsForThisVariety.length > 0 ? (
            selectedSeedsForThisVariety.map((seed) => (
              <Badge
                key={seed.id}
                variant="outline"
                className="bg-white border-slate-200 text-slate-600 text-[10px] py-0.5 px-2 rounded flex items-center gap-1 shadow-xs cursor-pointer hover:border-amber-300 transition-all"
                onClick={() => setDialogOpen(true)}
              >
  ...
  return (
    <Card className="border border-slate-200 shadow-xs rounded-xl overflow-hidden bg-white">
      <div className="p-4 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-amber-50 flex items-center justify-center shrink-0 border border-amber-100 overflow-hidden">
              <PawPrint className="w-4 h-4 text-amber-600" />
            </div>
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-xs text-muted-foreground mt-0.5">
                {varieties.length} giống vật nuôi khả dụng
              </span>
            </div>
          </div>
          <Button
        ...
        {loading ? (
          ...
          <div className="text-xs text-slate-400 italic">
            Không có giống vật nuôi nào khả dụng
          </div>
        )}
      </div>
    </Card>
...
export const ZoneConfigurationStep: React.FC<ZoneConfigurationStepProps> = ({
  ...
  const filteredRearingMethods = useMemo(() => {
    return rearingMethods.filter((item) => item.domainCode === "LIVESTOCK");
  }, [rearingMethods]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      {/* ── Farming Method & Water/Barn System (Horizontal top card) ── */}
      <Card className="border-none shadow-md bg-white">
        <CardHeader className="pb-3 border-b bg-linear-to-r from-amber-50/50 to-white">
              ...
              render={({ field }) => (
                <div className="space-y-2">
                  ...
                  <RemoteAutoCompleteSelect
                    ...
                    searchPlaceholder="Tìm kiếm phương pháp chăn nuôi..."
                    emptyText="Không tìm thấy phương pháp chăn nuôi"
              ...
              render={({ field }) => (
                <div className="space-y-2">
                  ...
                  <RemoteAutoCompleteSelect
                    ...
                    searchPlaceholder="Tìm kiếm hệ thống chuồng trại/cấp nước..."
        ...
        <Card className="border-none shadow-md bg-white flex flex-col">
          ...
          <CardContent className="pt-6 flex-1 flex flex-col min-h-0 space-y-6">
            {!selectedFarmingMethodId || selectedFarmingMethodId <= 0 ? (
              ...
              <>
                {/* Autocomplete crop/animal selector */}
                <div className="space-y-2">
                  <Label className="text-sm font-medium">Chọn vật nuôi</Label>
                  <RemoteAutoCompleteSelect
                    options={availableCropOptions}
                    value=""
                    onChange={handleSelectCrop}
                    onSearch={setCropSearch}
                    placeholder="Tìm kiếm và chọn vật nuôi..."
                    loading={fmcLoading}
                    emptyText="  Không tồn tại dữ liệu vật nuôi  "
                    clearable={false}
                  />
                </div>

                {/* List of Animal Subject Cards */}
                {fmcLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center border-2 border-dashed rounded-xl bg-slate-50 text-slate-400 gap-2 py-10">
                    <PawPrint className="w-8 h-8 opacity-40 text-amber-500" />
                    <span className="text-xs italic text-center px-4">
                      Chưa chọn vật nuôi nào
                    </span>
                  </div>
                )}
              </>
