...
function getAddDialogState(node: TreeModel): TreeDialogState | null {
  ...
  if (node.kind === "root") {
    return {
      ...
      subtitle: `Chọn các vùng chăn nuôi để thêm vào ${node.title}`,
...
function buildTreeModel(
  ...
  const root: TreeModel = {
    ...
    actions: makeActions(
      setLocation,
      `/animal-husbandry-region/${cultivationRegionId}`,
      `/animal-husbandry-region/${cultivationRegionId}/edit`,
    ),
...
export default function CultivationRegionWorkflowPage() {
  ...
  const flowDefinition = useMemo(() => {
    ...
    const root = buildTreeModel(
      selectedRegion.name || "Vùng chăn nuôi",
      ...
      actions={
        <div className="flex flex-wrap gap-2">
          <Button
            variant="outline"
            className="h-9 px-3"
            onClick={() => setLocation("/animal-husbandry-region")}
          >
          ...
          <Button
            variant="outline"
            className="h-9 px-3"
            onClick={() => setLocation(`/animal-husbandry-region/${params.id}`)}
          >
