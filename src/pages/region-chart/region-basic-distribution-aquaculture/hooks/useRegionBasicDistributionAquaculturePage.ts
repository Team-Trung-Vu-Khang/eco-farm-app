...
export function useRegionBasicDistributionAquaculturePage() {
  ...
  const openDetail = (id: number) => {
    setLocation(`/cultivation-region-identification/aquaculture/detail/${id}`);
  };
  ...
  const handleAdd = () => {
    setLocation("/cultivation-region-identification/aquaculture/create");
  };

  const handleEdit = (id: number) => {
    setLocation(`/cultivation-region-identification/aquaculture/edit/${id}`);
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast({ title: "Thành công", description: "Đã xóa vùng nuôi trồng thuỷ sản" });
    } catch (error) {
      toast({
        title: "Lỗi",
        description: "Không thể xóa vùng nuôi trồng thuỷ sản",
        variant: "destructive",
      });
    } finally {
      setDeleteOpen(false);
