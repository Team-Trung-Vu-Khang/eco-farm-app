    ...
    if (isEditMode) {
      if (zoneData && (!regionId || regionDataResponse)) {
        ...
        reset({
          ...
          irrigationSystemId: zoneData?.irrigationSystem?.id || undefined,
  ...
  const handleComplete = async (data: RegionBasicFormValues) => {
    ...
    try {
      ...
      const zoneChanged =
        ...
        zoneData.irrigationSystem?.id !== data.irrigationSystemId ||
      ...
      if (!isEditMode) {
        ...
        toast({
          title: "Thành công",
          description:
            "Đã tạo mới vùng chăn nuôi và cấu hình chăn nuôi thành công",
        });
      } else {
        ...
        toast({
          title: "Thành công",
          description: "Cập nhật vùng chăn nuôi thành công",
        });
      }

      setLocation("/cultivation-region-identification/animal");
    } catch (error) {
