...
import {
  MATERIAL_GROUP_FORM_STATUSES,
  materialGroupFormSchema,
  type MaterialGroupFormInput,
  type MaterialGroupFormValues,
} from "../data/material-group-form.schema";
import { emptyMaterialGroupFormData } from "../data/constants";
...
function normalizeStatus(
  status: MaterialGroupRecord["status"] | null | undefined,
): MaterialGroupFormValues["status"] {
  if (MATERIAL_GROUP_FORM_STATUSES.includes(status as never)) {
    return status as MaterialGroupFormValues["status"];
  }
...
export function MaterialGroupFormDialog({
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<MaterialGroupFormInput, unknown, MaterialGroupFormValues>({
    defaultValues,
  ...
  const submitForm: SubmitHandler<MaterialGroupFormValues> = (values) => {
    onSubmit({
      ...values,
