...
interface Step3ConfirmationProps {
  ...
  /** Có khi bước 2 dùng "Upload danh sách" — hiển thị kết quả file thay vì bảng cây */
  bulkUploadSummary?: {
    fileName: string;
    scopeName: string;
    varietyName: string;
...
export const Step3Confirmation: React.FC<Step3ConfirmationProps> = ({
  ...
  const plantCount = bulkUploadSummary
    ? (bulkUploadSummary.result?.successRows ?? 0)
    : plants.length;
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 max-w-4xl mx-auto">
      <div className="bg-green-50 border border-green-100 rounded-2xl p-8 text-center relative overflow-hidden">
        ...
        <p className="text-green-700/80 mt-2 z-10 relative max-w-lg mx-auto">
          ...
          <span className="font-bold">{plantCount} cây trồng</span>{" "}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="bg-white border rounded-xl p-4 text-center shadow-sm col-span-1 sm:col-span-1">
          <div className="text-2xl font-bold text-primary">{plantCount}</div>
      ...
      {bulkUploadSummary ? (
        <Card className="border-slate-200 shadow-sm overflow-hidden">
          <div className="bg-slate-50 border-b p-4 flex items-center gap-2">
            ...
            <h4 className="font-semibold text-slate-800">
              File danh sách cây trồng
            </h4>
          </div>
          <div className="p-6 space-y-4 bg-white">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              {[
                { label: "File", value: bulkUploadSummary.fileName },
                { label: "Phạm vi", value: bulkUploadSummary.scopeName },
                { label: "Giống cây", value: bulkUploadSummary.varietyName },
              ].map((item) => (
                <div
                  key={item.label}
                  className="p-3 rounded-xl border bg-slate-50/50 min-w-0"
                >
            ...
            {bulkUploadSummary.result && (
              <div className="flex flex-wrap gap-2 text-xs font-semibold">
                <span className="rounded-full bg-slate-100 px-2.5 py-1 text-slate-700">
                  Tổng {bulkUploadSummary.result.totalRows} dòng
                </span>
                <span className="flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-emerald-700">
                  ...
                  {bulkUploadSummary.result.successRows}
                </span>
                {bulkUploadSummary.result.failedRows > 0 && (
                  <span className="flex items-center gap-1 rounded-full bg-red-50 px-2.5 py-1 text-red-700">
                    ...
                    {bulkUploadSummary.result.failedRows}
                  </span>
                )}
              </div>
            )}
