...
// Positions are placeholders — useAquacultureWorkflowDraftStore auto-lays out the
...
function getMaterialBreakdown(
  ...
  materialNames.forEach((materialName, index) => {
    ...
    if (category.includes("chế phẩm") || category.includes("xử lý nước")) {
      breakdown["Chế phẩm xử lý nước"] += 1;
    } else if (category.includes("giống")) {
      breakdown["Con giống"] += 1;
    } else {
...
function getRegionLabelsFromPlan(
  ...
  if (!summary.length) return ["Chưa chọn ao nuôi"];
    ...
    data: {
      ...
      summaries: [
        { label: "Nhân lực", value: String(laborCount) },
        {
          ...
          value: String(materialBreakdown["Chế phẩm xử lý nước"]),
        },
        { label: "Con giống", value: String(materialBreakdown["Con giống"]) },
        {
          label: "Vật tư khác",
...
export default function PlanAquacultureGrowthCreateWorkflowPage() {
  ...
  const nodes = useAquacultureWorkflowDraftStore((state) => state.nodes);
  const edges = useAquacultureWorkflowDraftStore((state) => state.edges);
  const onNodesChange = useAquacultureWorkflowDraftStore(
    (state) => state.onNodesChange,
  );
  const onEdgesChange = useAquacultureWorkflowDraftStore(
    (state) => state.onEdgesChange,
  );
  const setEdges = useAquacultureWorkflowDraftStore((state) => state.setEdges);
  const addNode = useAquacultureWorkflowDraftStore((state) => state.addNode);
  const addNodeWithEdge = useAquacultureWorkflowDraftStore(
    (state) => state.addNodeWithEdge,
  );
  const removeNodeCascade = useAquacultureWorkflowDraftStore(
  ...
  const handleSavePlan = () => {
    ...
    setLocation(`/plan-aquaculture-growth/${plan.id}/workflow`);
  };
  ...
  return (
    <PageWrapper
      title="Tạo sơ đồ quy trình nuôi trồng thủy sản"
