...
interface DocumentationStepProps {
  ...
  handleUpdateDocs: (docKey: "farmingTechnique" | "qualityStandard", updates: any) => void;
}

export function DocumentationStep({ formData, handleUpdateDocs }: DocumentationStepProps) {
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="relative overflow-hidden rounded-xl border border-purple-200 bg-linear-to-r from-purple-50 via-white to-purple-50 p-6">
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900">Hệ thống tài liệu</h3>
      ...
      <div className="space-y-12">
        {(["farmingTechnique", "qualityStandard"] as const).map((docKey) => {
              ...
              >
                <div
                  className={cn(
                    ...
                    isFarming ? "bg-green-50 text-green-600" : "bg-blue-50 text-blue-600"
                  )}
                >
                  {isFarming ? <Leaf className="h-6 w-6" /> : <ShieldCheck className="h-6 w-6" />}
                </div>
                <div>
                ...
                {doc.type === "editor" ? (
                  <Card className="overflow-hidden border-2 focus-within:border-green-500/50 transition-all shadow-sm">
                    <Editor
                      ...
                      onSerializedChange={(content) => handleUpdateDocs(docKey, { content })}
                    />
                  </Card>
                ) : (
                  <div className="border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center bg-muted/20 hover:bg-muted/30 transition-colors cursor-pointer group">
                    ...
                    <p className="text-sm font-bold text-foreground">Click để tải lên PDF</p>
