...
export function UpcomingTasks() {
  // Call API for farm tasks (GET /api/farm/tasks?page=0&size=10)
  const { items: apiItems, loading: isLoading } = useFarmTasks({
  ...
  const taskEntries = useMemo(() => {
    if (apiItems && apiItems.length > 0) {
      return apiItems.map((item) => {
        const supplies = (item.supplyLines || []).map(
          (s) =>
            `${s.quantity || 1} ${s.unitName || s.unit || "đơn vị"} ${
              s.supplyItemName || s.name || s.supplyItem?.name || "Vật tư"
            }`,
        );

        return {
          id: item.id,
          purpose: (item.plan?.purpose as FarmPlanPurpose) || "CULTIVATION",
          name:
            item.name ||
            item.stage?.name ||
            item.plan?.name ||
            `Công việc #${item.code || item.id}`,
          ...
          description:
            ...
            item.stage?.name ||
            item.plan?.name ||
            "Đã phân công hạng mục công việc",
          imageCount: 0,
          stagesCount: 1,
          materialSummary:
            supplies.length > 0
      ...
      <CardContent className="p-4 space-y-3.5">
        {isLoading ? (
          <div className="py-8 text-center text-xs text-slate-400 italic">
            Đang tải dữ liệu công việc sắp tới...
          </div>
          ...
          <div className="py-10 text-center space-y-2">
            ...
            <p className="text-xs font-semibold text-slate-500">
              Chưa có công việc sắp tới
            </p>
            <p className="text-[11px] text-slate-400">
              Các công việc kế hoạch sắp tới sẽ hiển thị tại đây
            </p>
          </div>
        ) : (
          taskEntries.map((item) => {
            ...
            return (
              <div
                ...
                className="p-3.5 rounded-xl border border-slate-200/70 hover:border-blue-500 hover:bg-blue-50/30 transition-all cursor-pointer group space-y-2.5 bg-white shadow-2xs"
              >
