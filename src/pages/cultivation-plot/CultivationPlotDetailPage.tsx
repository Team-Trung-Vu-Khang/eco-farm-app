  ...
  const geometry = useMemo(() => {
    ...
    for (const region of regions) {
      for (const area of region.subAreas || []) {
        const found = (area.plots || []).find((p: any) => p.id === data.plotId || p.id === data.id);
    ...
    <AdminLayout
      ...
      actions={
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setLocation("/cultivation-plot")}>
            <ChevronLeft className="w-4 h-4 mr-2" />
            Quay lại
          </Button>
          <Button onClick={() => setLocation(`/cultivation-plot/${data.id}/edit`)}>
          ...
          <Card className="overflow-hidden border-none shadow-xl rounded-2xl">
            ...
            <CardContent className="bg-slate-50 p-6 flex flex-wrap gap-8 justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Vùng trồng</div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Khu vực</div>
                  <div className="font-bold text-slate-700">{data.areaName}</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Quản lý</div>
                         ...
                         <tr key={cropId} className="hover:bg-slate-50/50 transition-colors">
                           ...
                           <td className="px-6 py-4">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-xs font-medium text-slate-600">Đang trồng</span>
                              </div>
                           </td>
                         </tr>
                       );
          ...
          <Card className="border-none shadow-xl rounded-2xl bg-primary text-white overflow-hidden relative group">
             ...
             <CardContent className="p-6 relative z-10">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <div className="text-white/60 text-[10px] font-black uppercase tracking-widest">Phương pháp canh tác</div>
                    <div className="font-black text-lg leading-tight uppercase tracking-tight">{farmingMethod?.name || "VietGAP"}</div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                   <div className="bg-white/10 rounded-xl p-3 border border-white/10 backdrop-blur-sm">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[10px] text-white/70 font-bold uppercase">Hệ thống tưới</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Droplets className="w-3.5 h-3.5 text-blue-300" />
                        <span className="text-sm font-bold truncate">{irrigationSystem?.name || "Tưới nhỏ giọt Israel"}</span>
                      </div>
                   </div>

                   <div className="bg-white/10 rounded-xl p-3 border border-white/10 backdrop-blur-sm">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[10px] text-white/70 font-bold uppercase">Chứng nhận áp dụng</span>
                      </div>
                      <div className="flex flex-wrap gap-1.5">
                        {data.certificateIds.map(cid => (
                          <Badge key={cid} className="bg-white/20 hover:bg-white/30 text-white border-none text-[10px]">
          ...
          <Card className="border-none shadow-xl rounded-2xl overflow-hidden">
            <CardContent className="p-0">
               <div className="p-5 border-b bg-slate-50/30">
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Thông tin bổ sung</div>
                  <div className="space-y-4">
                     <div className="flex items-center gap-3">
                        <Calendar className="w-4 h-4 text-slate-400" />
                        <div>
                          <p className="text-[10px] font-bold text-slate-400 uppercase leading-none mb-1">Ngày tạo thiết lập</p>
                     ...
                     <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[10px] font-bold text-slate-400 uppercase leading-none mb-1">Trạng thái lô</p>
                          <Badge variant={data.status === "active" ? "default" : "secondary"} className="h-5 text-[10px] uppercase font-bold px-2">
