...
// Positions are placeholders — useAnimalGrowthWorkflowDraftStore places each
// new node next to its siblings after every add/remove, so the exact initial
...
export default function PlanAnimalGrowthCreateWorkflowPage() {
  ...
  const onNodesChange = useAnimalGrowthWorkflowDraftStore(
    (state) => state.onNodesChange,
  );
  const onEdgesChange = useAnimalGrowthWorkflowDraftStore(
    (state) => state.onEdgesChange,
  );
  const setEdges = useAnimalGrowthWorkflowDraftStore((state) => state.setEdges);
  const addNode = useAnimalGrowthWorkflowDraftStore((state) => state.addNode);
  const addNodeWithEdge = useAnimalGrowthWorkflowDraftStore(
  ...
  const handlers: NodeHandlers = {
    ...
    onEditPlan: (planId) =>
      setLocation(`/plan-animal-growth/create/workflow/plan/${planId}/edit`),
  ...
  const handleSavePlan = () => {
    ...
    setLocation(`/plan-animal-growth/${plan.id}/workflow`);
  };
  ...
  return (
    <PageWrapper
      title="Tạo sơ đồ quy trình chăn nuôi"
      description="Quy trình triển khai các kế hoạch chăn nuôi được liên kết với nhau trên sơ đồ."
      actions={
        <div className="flex flex-wrap gap-2">
          <Button
            variant="outline"
            className="h-9 px-3"
            onClick={() => setLocation("/plan-animal-growth")}
          >
