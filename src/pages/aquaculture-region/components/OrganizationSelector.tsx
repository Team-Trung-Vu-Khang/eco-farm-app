...
export const OrganizationSelector = ({
  ...
  selectedId: string;
  onSelect: (id: string, organization?: OrganizationRecord) => void;
  disabled?: boolean;
}) => {
  ...
  const { items: filteredOrganizations, loading: isLoadingSearch } =
    useOrganizations(
      {
  ...
  const { item: selectedOrganization, loading: isLoadingSelected } =
    useOrganizationById(
      selectedId,
      parsedWorkspaceId ?? "missing",
      { enabled: parsedWorkspaceId !== undefined && !!selectedId },
    );

  const handleConfirm = () => {
    const selectedOrgFromList = filteredOrganizations.find(
      (org) => org.id.toString() === tempSelectedId
    );
    const orgToPass = selectedOrgFromList || (tempSelectedId === selectedId ? selectedOrganization : undefined) || undefined;
    
    onSelect(tempSelectedId, orgToPass);
    setIsOpen(false);
  };
  ...
  return (
    <>
      ...
      >
        {selectedOrganization && !isLoadingSelected ? (
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground flex items-center gap-1.5">
                  ...
                  <span>{selectedOrganization.taxCode || "N/A"}</span>
                </div>
              </div>
              {!disabled && (
                <Button
            ...
            <div className="grid grid-cols-1 gap-2 pt-3 border-t border-slate-100">
              <div className="flex items-start gap-2.5 text-xs text-slate-600">
                ...
                <div className="leading-relaxed">
                  ...
                  {selectedOrganization.address || "N/A"}
                </div>
              </div>
            </div>
          </div>
        ) : isLoadingSelected ? (
          <div className="flex flex-col items-center justify-center w-full gap-3 text-muted-foreground py-4">
          ...
          <div className="space-y-4 py-4">
            ...
            <ScrollArea className="h-100 border rounded-xl bg-slate-50/50">
              <div className="p-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {!isLoadingSearch && filteredOrganizations.length === 0 && (
