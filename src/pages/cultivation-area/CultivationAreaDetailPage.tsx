...
const CultivationAreaDetailPage = () => {
  ...
  const data = useMemo(() => {
    ...
    return getCultivationAreaById(areaId);
  ...
  const geometry = useMemo(() => {
    ...
    for (const region of regions) {
      if (region.id.toString() === data.regionId) {
        const found = (region.subAreas || []).find((a: any) => a.id.toString() === data.areaId || a.id === data.id);
  ...
  const enterprise = enterprises.find((e: any) => e.id.toString() === data.enterpriseId);
  const manager = personnel.find((m: any) => m.id.toString() === data.managerId);
  const farmingMethod = farmingMethods.find((m: any) => m.id === data.farmingMethodId);
  const irrigationSystem = irrigationSystems.find((m: any) => m.id === data.irrigationMethodId);
    ...
    <AdminLayout
      ...
      actions={
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setLocation("/cultivation-area")}>
            <ChevronLeft className="w-4 h-4 mr-2" />
            Quay lại
          </Button>
          <Button onClick={() => setLocation(`/cultivation-area/${data.id}/edit`)}>
          ...
          <Card className="overflow-hidden border-none shadow-xl rounded-2xl">
            <div className="h-[400px] relative">
              ...
              >
                ...
                {geometry?.coordinates && (
                  <Polygon
                    positions={geometry.coordinates.map((c: any) => [c.lat, c.lng])}
                    pathOptions={{
                      color: "#10b981",
                      weight: 3,
                      fillColor: "#10b981",
            ...
            <CardContent className="bg-slate-50 p-6 flex flex-wrap gap-8 justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Vùng trồng cha</div>
                  <div className="font-bold text-slate-700">{data.regionName}</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Mã khu vực</div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Quản lý khu vực</div>
                  <div className="font-bold text-slate-700">{manager?.fullName || "Chưa gán"}</div>
                </div>
              </div>
            </CardContent>
          </Card>
                 ...
                 <table className="w-full text-sm text-left">
                   ...
                   <tbody className="divide-y divide-slate-100">
                     {data.selectedCrops.map((cropId) => {
                       ...
                       return (
                         <tr key={cropId} className="hover:bg-slate-50/50 transition-colors">
                           <td className="px-6 py-4">
                             <div className="font-bold text-slate-900">{v?.varietyName}</div>
                             <div className="text-[10px] text-slate-400 font-mono italic">{v?.crop}</div>
                           </td>
                           <td className="px-6 py-4">
                           ...
                           <td className="px-6 py-4">
                             <div className="flex flex-wrap gap-1">
                               {seedIds.length > 0 ? (
                                 seedIds.map(sid => (
                                   <Badge key={sid} variant="secondary" className="text-[10px] py-0 px-1.5 h-5">#{sid}</Badge>
                                 ))
                               ) : (
                                 <span className="text-slate-400 italic text-xs">Mặc định</span>
                               )}
                             </div>
                           </td>
                           <td className="px-6 py-4">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-xs font-medium text-slate-600">Đang canh tác</span>
                              </div>
                           </td>
                         </tr>
                       );
          ...
          <Card className="border-none shadow-xl rounded-2xl bg-slate-900 text-white overflow-hidden relative group">
             ...
             <CardContent className="p-6 relative z-10">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <div className="text-white/60 text-[10px] font-black uppercase tracking-widest">Tiêu chuẩn canh tác</div>
                    <div className="font-black text-lg leading-tight uppercase tracking-tight text-primary">{farmingMethod?.name || "Organic Standard"}</div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                   <div className="bg-white/5 rounded-xl p-3 border border-white/10 backdrop-blur-sm">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[10px] text-white/50 font-bold uppercase">Hệ thống cấp nước</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Droplets className="w-3.5 h-3.5 text-blue-400" />
                        <span className="text-sm font-bold truncate">{irrigationSystem?.name || "Hệ thống tưới tự động"}</span>
                      </div>
                   </div>

                   <div className="bg-white/5 rounded-xl p-3 border border-white/10 backdrop-blur-sm">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[10px] text-white/50 font-bold uppercase">Chứng nhận khu vực</span>
                      </div>
                      <div className="flex flex-wrap gap-1.5">
                        {data.certificateIds.map(cid => (
                          <Badge key={cid} className="bg-primary/20 hover:bg-primary/30 text-primary border-none text-[10px]">
            ...
            <CardContent className="pt-4 space-y-4">
               <div className="flex items-start gap-4 p-3 rounded-xl border border-slate-100 hover:bg-slate-50 transition-colors group">
                  <div className="w-12 h-12 rounded-xl bg-white overflow-hidden flex items-center justify-center shrink-0 shadow-sm border border-slate-200">
                    <img src={enterprise?.image || "https://github.com/shadcn.png"} alt="Enterprise" className="w-full h-full object-cover" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="font-bold text-slate-800 text-sm truncate">{enterprise?.name}</div>
                    <div className="text-[10px] text-slate-400 font-medium truncate mt-0.5">{enterprise?.address || "Khu vực kinh doanh"}</div>
                    <Link href={`/enterprise/${enterprise?.id}`}>
                      <a className="text-[10px] text-primary font-bold hover:underline flex items-center gap-1 mt-2 uppercase">
                        Xem chi tiết doanh nghiệp <ExternalLink size={8} />
                      </a>
                    </Link>
                  </div>
               </div>
          ...
          <Card className="border-none shadow-xl rounded-2xl overflow-hidden">
            <CardContent className="p-0">
               <div className="p-5 border-b bg-slate-50/30">
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Lịch sử & Trạng thái</div>
                  <div className="space-y-5">
                     <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-[10px] font-bold text-slate-400 uppercase leading-none mb-1.5">Ngày khởi tạo vùng</p>
                          <p className="text-sm font-bold text-slate-700">{data.createdAt}</p>
                        </div>
                     </div>
                     <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-[10px] font-bold text-slate-400 uppercase leading-none mb-1.5">Trạng thái hiện tại</p>
                          <Badge variant={data.status === "active" ? "default" : "secondary"} className="h-5 text-[10px] uppercase font-bold px-2 bg-emerald-500">
                             {data.status === "active" ? "Đang hoạt động" : "Tạm ngưng"}
                          </Badge>
                        </div>
                     </div>
                  </div>
