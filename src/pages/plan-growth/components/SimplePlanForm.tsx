...
function StageMaterialPicker({
  ...
  const materialOptions = useMemo(() => {
    const options = searchedMaterials.map(mapSupplyItemToOption);
    if (
      selectedMaterial &&
      !options.some((option) => option.value === String(selectedMaterial.id))
    ) {
      options.unshift(mapSupplyItemToOption(selectedMaterial));
    }
    return options;
