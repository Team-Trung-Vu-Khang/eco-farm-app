...
export default function PlanGrowthWorkflowDetailEditPage() {
  const [, setLocation] = useLocation();
  const { toast } = useToast();
  const params = useParams();
  const nodeId = params.nodeId as string;

  const node = usePlanWorkflowDraftStore((state) =>
    state.nodes.find((item) => item.id === nodeId),
  );
  const updateNodePayload = usePlanWorkflowDraftStore((state) => state.updateNodePayload);
