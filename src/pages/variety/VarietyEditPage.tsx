...
export default function VarietyEditPage() {
  ...
  const { getVarietyById, updateVariety } = useVarietyStore();

  const existingVariety = useMemo(
    () => getVarietyById(id!),
    [id, getVarietyById],
  );
  ...
  const [formData, setFormData] = useState<CreateVarietyForm>({
    ...
    illustration: (existingVariety?.illustration as File) || undefined,
            ...
            <div className="relative z-10 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-amber-700/80">
                  Cập nhật các thông tin chi tiết về đặc tính và hình ảnh giống
                </p>
              </div>
            </div>
