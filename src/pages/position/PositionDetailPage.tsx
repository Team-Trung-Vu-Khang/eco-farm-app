...
export default function PositionDetailPage() {
  const [, params] = useRoute("/position/:id/detail");
  const [, setLocation] = useLocation();
  const id = params?.id ?? "";

  const positionQuery = useMasterDataById("positions", id);
  const position = positionQuery.data as PositionRecord | undefined;

  const documents = useMemo(() => position?.documents ?? [], [position]);
    ...
    return (
      <AdminLayout title="Chi tiết chức vụ">
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <Button variant="outline" onClick={() => setLocation("/position")}>
