...
const BiologicalProductDetailPage = () => {
  const [matchFarm, paramsFarm] = useRoute(
    "/cultivation-material/biological-product/:id",
  );
  const [matchAdmin, paramsAdmin] = useRoute("/admin/biological-product/:id");
  ...
  if (loading) {
    return (
      <PageWrapper title="Chi tiết chế phẩm sinh học">
  ...
  if (!item) {
    return (
      <PageWrapper
        title="Chi tiết chế phẩm sinh học"
        description="Đang tải thông tin..."
      >
        <div className="flex flex-col items-center justify-center py-20">
          <p className="text-muted-foreground mb-4">
            Không tìm thấy thông tin chế phẩm sinh học.
          </p>
          <Button
            onClick={() =>
              setLocation(
          ...
          <Card className="shadow-sm border-slate-100 bg-white">
            ...
            <CardContent className="pt-5 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <InfoRow
                  label="Số đăng ký quyết định lưu hành"
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.targetCrops.map((crop) => (
                      <Badge
                        key={crop}
                        variant="outline"
                        className="text-xs border-green-200 bg-green-50/30 text-green-800"
                      >
                        {crop}
                      </Badge>
                    ))}
                    ...
                    {item.packagingVariants && item.packagingVariants.length > 0
                      ...
                      : item.packagingSpecs?.map((spec) => (
                          <Badge
                            key={spec}
                            variant="secondary"
                            className="text-xs px-2.5 py-1 flex items-center gap-1.5 bg-slate-100 border-slate-200"
                          >
...
export default BiologicalProductDetailPage;
