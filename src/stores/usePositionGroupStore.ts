...
interface PositionGroupStore {
  positionGroups: PositionGroup[];

  // CRUD
  getPositionGroupById: (id: number) => PositionGroup | undefined;
  addPositionGroup: (group: Omit<PositionGroup, "id" | "createdAt">) => void;
  updatePositionGroup: (
    id: number,
    updates: Partial<Omit<PositionGroup, "id" | "createdAt">>,
  ) => void;
  deletePositionGroup: (id: number) => void;
}

const INITIAL_GROUPS: PositionGroup[] = [
  {
    id: 1,
    code: "GRP-MNG",
...
const usePositionGroupStore = create<PositionGroupStore>()(
  devtools(
    persist(
      (set, get) => ({
        positionGroups: INITIAL_GROUPS,

        getPositionGroupById: (id) =>
          get().positionGroups.find((g) => g.id === id),

        addPositionGroup: (groupData) =>
          set(
            (state) => {
              const newId =
                state.positionGroups.length > 0
                  ? Math.max(...state.positionGroups.map((g) => g.id)) + 1
                  : 1;
              ...
              return {
                positionGroups: [...state.positionGroups, newGroup],
              };
            },
            false,
            "addPositionGroup",
        ...
        updatePositionGroup: (id, updates) =>
          set(
            (state) => ({
              positionGroups: state.positionGroups.map((g) =>
                g.id === id ? { ...g, ...updates } : g,
              ),
            }),
            false,
        ...
        deletePositionGroup: (id) =>
          set(
            (state) => ({
              positionGroups: state.positionGroups.filter((g) => g.id !== id),
            }),
            false,
            "deletePositionGroup",
          ),
      ...
      {
        name: "position-group-storage", // Key trong localStorage
        partialize: (state) => ({ positionGroups: state.positionGroups }),
      },
    ),
    {
      name: "PositionGroupStore", // Tên hiển thị trong Redux DevTools
    },
  ),
);

export default usePositionGroupStore;
